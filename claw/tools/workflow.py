...
class WorkflowTool(Tool):
    ...
    wants_progress = True
    # Receives the outer turn's absolute deadline, so its own multi-step budget
    # is bounded by whatever's actually left on the parent turn's clock.
    wants_deadline = True
    description = (
    ...
    parameters = {
        "type": "object",
        "properties": {"request": {"type": "string", "description": "The complex, multi-step request"}},
        "required": ["request"],
    }
    ...
    async def execute(
        ...
        **_: Any,
        ...
        result = await self.service.run_request(request, on_progress=on_progress, outer_deadline=deadline)
