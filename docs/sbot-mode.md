...
Docker Compose production configuration mounts both workspace roots at identical host/container paths and persists Blueprint storage. `CLAW_SANDBOX__PROJECTS_ENABLED=false` by default. Build `docker/developer.Dockerfile` as `sbot-developer:latest` before enabling projects. Nested Compose is separately opt-in through `CLAW_SANDBOX__PROJECT_DOCKER_ENABLED`; use only trusted workloads on an isolated development host. Project policies must also allow the user/group.
