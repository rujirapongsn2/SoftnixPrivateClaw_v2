...
from sbot.channels.telegram import validate_bot_token
from sbot.db.models import User
from sbot.db.stores import plan_to_dict
from sbot.notifications.mailer import SmtpSendError, send_email
from sbot.security.policy import DEFAULT_TOOL_ARGS_EXEMPT, rule_from_row
...
class UserProjectPolicyBody(BaseModel):
    """None/None inherits the group; True requires an explicit positive cap."""

    enabled: bool | None = None
    max_containers: int | None = Field(default=None, ge=0, le=100)

    @model_validator(mode="after")
    def _valid(self):
        if self.enabled is None and self.max_containers is not None:
            raise ValueError("an inherited project policy cannot set a container limit")
        if self.enabled is True and (self.max_containers is None or self.max_containers < 1):
            raise ValueError("an enabled project policy needs a limit of at least 1")
        return self
...
async def update_user(
    ...
    if body.project_policy is not None:
        updated = await state.users.set_project_policy(
...
# ---------------------------------------------------------------- user groups
# Groups provide inherited plan and project-container policy defaults.
...
class GroupPatchBody(BaseModel):
    ...
    plan_id: str | None = "__unset__"
    project_policy: "GroupProjectPolicyBody | None" = None


class GroupProjectPolicyBody(BaseModel):
    enabled: bool
    max_containers: int = Field(ge=0, le=100)

    @model_validator(mode="after")
    def _valid(self):
        if self.enabled and self.max_containers < 1:
            raise ValueError("an enabled project policy needs a limit of at least 1")
        return self
...
def _group_row(g, counts: dict[str, int], plan_names: dict[str, str]) -> dict:
    return {
        ...
        "project_containers_enabled": g.project_containers_enabled,
        "project_container_limit": g.project_container_limit,
    }
...
async def update_group(
    ...
    if body.project_policy is not None:
        updated = await state.groups.set_project_policy(
            group_id, body.project_policy.enabled, body.project_policy.max_containers
        )
    if updated is None:
        raise HTTPException(status_code=404, detail="group not found")
    counts = await state.groups.counts_by_group()
...
async def _telegram_config_json(state: AppState) -> dict:
    ...
    if cfg is None:
        ...
        # fallback (SBOT_TELEGRAM_BOT_TOKEN) currently supplies a token, so the
...
async def test_email_config(
    ...
    try:
        await send_email(
            cfg,
            body.recipient,
            subject="Sbot test email",
            text_body=(
                "This is a test email from your Sbot Control Plane's "
