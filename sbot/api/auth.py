...
from sbot.auth.password_reset import make_password_reset_token, verify_password_reset_token
from sbot.auth.passwords import hash_password, verify_password
from sbot.auth.tokens import create_access_token
from sbot.db.models import User
from sbot.notifications.mailer import send_email
...
async def _send_activation_email(state: AppState, user_id: str) -> str:
    ...
    try:
        ...
        await send_email(
            smtp,
            user.email,
            subject="Activate your Sbot account",
            text_body=(
                f"Hi {user.display_name or user.email},\n\n"
                "An administrator has added you to Sbot. Click the link below "
...
async def _send_activation_confirmed_email(state: AppState, user_id: str) -> None:
    ...
    try:
        ...
        await send_email(
            smtp,
            user.email,
            subject="Your Sbot account is now active",
            text_body=(
                f"Hi {user.display_name or user.email},\n\n"
                "Your Sbot account was just activated. If this wasn't "
...
async def _send_password_reset_email(state: AppState, user_id: str) -> str:
    ...
    try:
        ...
        await send_email(
            smtp,
            user.email,
            subject="Reset your Sbot password",
            text_body=(
                f"Hi {user.display_name or user.email},\n\n"
                "We received a request to reset your Sbot password. Click "
...
async def _send_password_reset_confirmed_email(state: AppState, user_id: str) -> None:
    ...
    try:
        ...
        await send_email(
            smtp,
            user.email,
            subject="Your Sbot password was changed",
            text_body=(
                f"Hi {user.display_name or user.email},\n\n"
                "Your Sbot password was just changed. If this wasn't "
                "you, contact your administrator immediately."
            ),
        )
    except Exception:
