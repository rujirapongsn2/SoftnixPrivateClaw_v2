...
from sbot.db.stores import ConnectorStore
from sbot.security.ssrf import resolve_public_ips
...
def _register_scoped(registry: ToolRegistry, state, tool: Tool, connector_name: str) -> bool:
    ...
    Asks `is_registered`, not `has`: `has` answers for the *current bot's*
    allowlist, under which every connector tool name reads as absent, so the
    guard would be silently off for exactly the bots that are most restricted.
    """
...
def _populate(registry: ToolRegistry, state) -> None:
    """Mirror an already-connected user's tools into one more registry.

    Registration is per registry but `_UserConnections` is per user, so a
    second bot of the same user reaches sync_tools with everything already
    connected and nothing registered where it can see it. Pure in-memory
    re-registration of the same proxy objects — no session, no handshake, no
    teardown — which is also why it is safe to do inside the unchanged-signature
    fast path. `state.tool_names` is deliberately not appended to: these are the
    same names, now held in one more place.
    """
...
class _UserConnections:
    ...
    # The built tool objects behind `tool_names`, kept so an already-connected
    # set can be registered into another of the user's registries without
    # reconnecting anything (see _populate). One user has one connector set but
    # one registry PER BOT, and this state is cached per user — so without
    # this, the first bot to sync claimed the tools and every other bot of the
    # same user hit the unchanged-signature early return and got none.
    tools: list[Tool] = field(default_factory=list)
    # Registries these tools are currently registered in. Weak so an agent
    # evicted from the runtime's LRU is not kept alive by this cache, and so
    # `_close_user` only has to reach the registries that still exist.
...
class ConnectorManager:
    ...
    async def sync_tools(self, user_id: str, registry: ToolRegistry) -> None:
        ...
        async with self._lock(user_id):
            ...
            if (
                ...
                # Nothing to reconnect — but this may be a registry that has
                # never held these tools (another of the user's bots, or the
                # /connectors warm having primed the cache off a different
                # agent), and returning without looking would leave that
                # registry permanently empty for as long as the signature holds.
    ...
    def _build_api_tools(self, connector, *, connector_ref: Callable[[], Any] | None = None) -> list[Tool]:
        ...
        sbot.core <-> sbot.tools import cycle (connectors.py's _redact_secrets
    ...
    async def _connect(self, stack: AsyncExitStack, connector) -> Any:
        ...
        if connector.transport == "http":
            ...
            # Built-in preset servers launch as `python -m sbot.integrations.*`.
    ...
    async def _close_user(self, user_id: str, registry: ToolRegistry | None = None) -> None:
        ...
        if state is None:
            return
        # Every registry these tools were mirrored into, not just the caller's:
