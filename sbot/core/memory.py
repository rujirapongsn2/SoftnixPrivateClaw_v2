...
from sbot.db.stores import MemoryStore, MessageStore, SessionStore, UsageStore
from sbot.providers.base import LLMProvider
...
# Escalate repeated unusable summaries to an operator-visible error without
# advancing the consolidation cursor. Source requirements must remain readable.
...
def memory_scope(bot_id: str | None, is_cos: bool) -> str | None:
    """Which bot's private memory document a turn should read and write.

    Chief of Staff and the default agent return None — the shared user
    document. They are the user's primary interface, so what they learn ("the
    user's name is X") has to be visible to every specialist rather than
    trapped in one bot's doc.
    """
    return None if is_cos or not bot_id else bot_id
...
class MemoryService:
    ...
    async def build_context(self, user_id: str, bot_id: str | None = None) -> str:
        """Assemble the memory block for a turn.

        Two documents, not one. Facts about the user are shared, so a specialist
        does not have to relearn the user's name; the lessons a specialist draws
        from its own work are private to it, which is what makes each bot get
        better at its own role instead of averaging into a single voice
        (PRD §4.5). `bot_id` is None for the shared document alone.
        """
        shared = await self.memories.get_core(user_id)
        own = (
            await self.memories.get_core(user_id, scope="bot", scope_id=bot_id) if bot_id else ""
        )
        if not shared and not own:
            return ""
        # Demote every heading so each document nests under its own section
        ...
        parts = ["# Memory"]
        if shared:
            parts.append(f"## Long-term Memory\n{_demote_headings(shared)}")
        if own:
            parts.append(f"## What You Have Learned In This Role\n{_demote_headings(own)}")
        return "\n\n".join(parts)

    async def remember(self, user_id: str, fact: str, bot_id: str | None = None) -> str:
        ...
        scope: dict[str, Any] = {"scope": "bot", "scope_id": bot_id} if bot_id else {}
        current = await self.memories.get_core(user_id, **scope)
        ...
        await self.memories.set_core(user_id, updated, **scope)
        return f"Saved to memory: {fact}"

    async def core_text(self, user_id: str, bot_id: str | None = None) -> str:
        if bot_id:
            return await self.memories.get_core(user_id, scope="bot", scope_id=bot_id)
    ...
    def _note_failure(self, session_id: str) -> None:
        ...
        delay = min(_BACKOFF_BASE_SECONDS * 2 ** min(count - 1, 16), _BACKOFF_MAX_SECONDS)
    ...
    async def _note_unusable(self, session_id: str, cutoff: int, reason: str) -> bool:
        """Back off on unusable summaries without retiring unsummarized history."""
        ...
        if count < _MAX_POISON_ATTEMPTS:
            ...
            return False
        # Keep the cursor behind unsummarized history; never silently retire
        # requirements because the summarizer returned an unusable response.
        self._note_failure(session_id)
        logger.error(
            "Memory consolidation ({}): retaining messages through seq {} after {} unusable "
            "responses (last: {}); retry scheduled with backoff",
            session_id,
            cutoff,
            count,
            reason,
    ...
    async def maybe_consolidate(
        self, user_id: str, session_id: str, bot_id: str | None = None
    ) -> bool:
        """Consolidate while enough unconsolidated messages remain. Returns True if any pass ran."""
        # Keyed by the document, not the session: a user with two live sessions
        # (web + Telegram) writing the same doc would otherwise have both passes
        # read it and the slower write discard the faster. Two different bots
        # write different docs, so they are free to run at once. The slot is
        # taken once for the whole catch-up, not per pass, so a second session
        # cannot interleave into the middle of it.
        lock_key = f"{user_id}:{bot_id or 'shared'}"
        if lock_key in self._in_flight or self._backing_off(session_id):
            return False
        ...
        try:
            for _ in range(_MAX_CATCH_UP_PASSES):
                ...
                if not await self._consolidate(user_id, session_id, bot_id):
                    break
                ran = True
        except Exception:
    ...
    async def _consolidate(
        ...
        # A specialist's consolidation is a lesson about its own role, so it
        # lands in that bot's private doc; Chief of Staff and the default agent
        # (bot_id None) keep writing the shared one, which is where facts about
        # the user belong.
        scope: dict[str, Any] = {"scope": "bot", "scope_id": bot_id} if bot_id else {}
        current_memory = await self.memories.get_core(user_id, **scope)
        ...
        if not result.has_tool_calls:
            return await self._note_unusable(
                session_id, cutoff, "model did not call save_memory"
            )
