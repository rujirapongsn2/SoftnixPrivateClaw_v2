...
class MemoryTool(Tool):
    ...
    def __init__(self, memory: MemoryService, user_id: str, bot_id: str | None = None):
        self.memory = memory
        self.user_id = user_id
        # Which document this bot reads and writes: its own when set, the shared
        # user document when None (Chief of Staff and the default agent).
        self.bot_id = bot_id
    ...
    async def execute(self, action: str, **kwargs: Any) -> str:
        if action == "save":
            ...
            return await self.memory.remember(self.user_id, fact, bot_id=self.bot_id)
        if action == "list":
            core = await self.memory.core_text(self.user_id, bot_id=self.bot_id)
