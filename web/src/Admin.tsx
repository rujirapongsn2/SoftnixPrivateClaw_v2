...
function OverviewSummary({ data }: { data: AdminOverview }) {
  ...
  const maxHourlyActivity = Math.max(1, ...data.activity_by_day_hour_7d.map((point) => point.count));
  const activityDays = Array.from(new Set(data.activity_by_day_hour_7d.map((point) => point.label.slice(0, 10))))
    .map((date) => ({
      date,
      points: data.activity_by_day_hour_7d.filter((point) => point.label.startsWith(`${date}T`)),
    }));
    ...
    <Card padding={3} className="claw-summary-dashboard">
      ...
      <section className="claw-summary-activity" aria-labelledby="summary-activity-title">
        <div className="claw-summary-section-head">
          ...
          <Text size="sm" color="secondary">{t("admin.overview.summary.hourly7Days")}</Text>
        </div>
        ...
        <div className="claw-summary-week-heatmap" role="img" aria-label={t("admin.overview.summary.hourly7Days")}>
          {activityDays.map((day) => (
            <div className="claw-summary-week-row" key={day.date}>
              <span className="claw-summary-week-day">{day.date.slice(5)}</span>
              <div className="claw-summary-week-hours">
                {day.points.map((point) => (
                  <span
                    key={point.label}
                    title={`${day.date} ${point.label.slice(11)}:00 — ${point.count.toLocaleString()}`}
        ...
        <div className="claw-summary-week-axis" aria-hidden="true">
          <span />
          <div><span>00:00</span><span>06:00</span><span>12:00</span><span>18:00</span><span>23:00</span></div>
        </div>
      </section>
