...
function endpoint(path: string): string {
  return MODE_LOCAL.test(path) ? `/modes/sbot${path}` : path;
}
...
export function openChatSocket(sessionId: string): WebSocket {
  ...
  return new WebSocket(
    `${proto}://${location.host}/modes/sbot/ws/chat/${sessionId}?token=${encodeURIComponent(getToken())}`,
  );
}
...
export function fileUrl(sessionId: string, path: string): string {
  ...
  return `/modes/sbot/api/sessions/${sessionId}/files/${encoded}?token=${encodeURIComponent(getToken())}`;
}
